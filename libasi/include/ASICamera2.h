...
/**
 * Enum defining different operating modes for an ASI camera.
 */
typedef enum ASI_CAMERA_MODE {
    ASI_MODE_NORMAL = 0,             // Normal operating mode.
    ASI_MODE_TRIG_SOFT_EDGE,         // Triggered capture on soft rising edge.
    ASI_MODE_TRIG_RISE_EDGE,         // Triggered capture on rising edge.
    ASI_MODE_TRIG_FALL_EDGE,         // Triggered capture on falling edge.
    ASI_MODE_TRIG_SOFT_LEVEL,        // Triggered capture on soft signal level.
    ASI_MODE_TRIG_HIGH_LEVEL,        // Triggered capture on high signal level.
    ASI_MODE_TRIG_LOW_LEVEL,         // Triggered capture on low signal level.
    ASI_MODE_END = -1                // End marker for the enumeration.
} ASI_CAMERA_MODE;
...
ASI_SUCCESS : Operation is successful
ASI_ERROR_CAMERA_CLOSED : camera didn't open
ASI_ERROR_INVALID_SEQUENCE : camera is in capture now, need to stop capture first._ASI_CONTROL_CAPS
